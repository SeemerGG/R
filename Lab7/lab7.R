...
weightlifting<-df[which(df[, "Sport"]=="Weightlifting"), c("Name", "Sex", "Weight")]

# удаление строчек с пустым значением поля "Вес"
weightlifting<-weightlifting[-which(is.na(weightlifting[, "Weight"])),]

# удаление повторяющихся строчек
weightlifting<-unique(weightlifting)

# значения столбца "Вес" становятся вектором
x<-weightlifting[1:nrow(weightlifting), "Weight"]
...
# Тест Уилкоксона.
wilcox.test(x, mu=median(x), conf.int=TRUE)

#Задание 2.

wrest_gymnast<-df[which(df[, "Sport"]%in%c("Gymnastics", "Weightlifting")), c("Name", "Sex", "Weight", "Sport")]
...
bartlett.test(wrest_gymnast$Weight~wrest_gymnast$Sport, wrest_gymnast)
